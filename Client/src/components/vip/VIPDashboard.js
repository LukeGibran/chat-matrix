...
class VIPDashboard extends Component {
    ...
    componentDidMount() {
        ...
        axios
            .get(`${utils.serverURL}/v/vip/user`)
            .then((response) => {
  ...
  openSocketConnection = () => {
      const { isUserMediaAllowed, user, userIP, avatarID } = this.state;
  ...
  render() {
      ...
      if (!isUserMediaAllowed) {
          return (
              <UserMediaBlockedModal
                  show
                  onLogOut={this.handleLogOut}
                  isUserMediaAllowed={isUserMediaAllowed}
              />
          );
      }
