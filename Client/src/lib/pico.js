...
pico.unpack_cascade = function (bytes) {
	...
	*/
    ...
    for (var t = 0; t < ntrees; ++t) {
        ...
        Reflect.apply(Array.prototype.push, tcodes, [0, 0, 0, 0]);
        Reflect.apply(Array.prototype.push, tcodes, bytes.slice(p, p + 4 * Math.pow(2, tdepth) - 4));
    ...
    function classifyRegion(r, c, s, pixels, ldim) {
        ...
        for (var i = 0; i < ntrees; ++i) {
            ...
            for (var j = 0; j < tdepth; ++j) {
                ...
                idx = 2 * idx + (
                    pixels[((r + tcodes[root + 4 * idx + 0] * s) >> 8) *
                    ldim + ((c + tcodes[root + 4 * idx + 1] * s) >> 8)] <=
                    pixels[((r + tcodes[root + 4 * idx + 2] * s) >> 8) *
                    ldim + ((c + tcodes[root + 4 * idx + 3] * s) >> 8)]
                );
            }
